...
def compare_assessments(social_data: AssessmentInput, chat_data: AssessmentInput) -> Tuple[int, bool, List[str]]:
    ...
    """
    # 1. חישוב הציון הממוצע (חלוקה בשלמים)
    unified_score = (social_data.raw_score + chat_data.raw_score) // 2
    
    has_significant_gap = False
    gap_details = []
    
    # 2. מעבר על כל המדדים (Metrics) והשוואה ביניהם
    for metric_name, social_value in social_data.metrics.items():
        if metric_name in chat_data.metrics:
            chat_value = chat_data.metrics[metric_name]
            
            # חישוב הפער המוחלט (ללא מינוס)
            gap = abs(social_value - chat_value)
            
            # הגדרת סף לפער משמעותי (מעל 15 נקודות נחשב חריג)
            if gap >= 15:
